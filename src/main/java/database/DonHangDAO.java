...
public class DonHangDAO implements DAOInterface<DonHang> {
	...
	public ArrayList<DonHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				DonHang dh = new DonHang(maDH, khachHang, diaChiMuaHang, diaChiNhanHang, trangThaiThanhToan, hinhThucThanhToan, trangThaiThanhToan,
	...
	public DonHang selectById(DonHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				KhachHang khachHang = new KhachHangDAO()
						.selectById(new KhachHang(maKH, "", "", "", "", "", "", "", null, "", "", false));
				DonHang dh = new DonHang(maDH, khachHang, diaChiMuaHang, diaChiNhanHang, trangThaiThanhToan, hinhThucThanhToan, 
	...
	public int insert(DonHang t) {
		int kq = 0;
		Connection con = JDBCUtil.getConnection();
		String sql = "INSERT INTO donhang(madonhang, khachhang, diachimuahang, diachinhanhang, hinhthucthanhtoan, trangthaithanhtoan,sotiendathanhtoan, sotienconthieu,ngaydathang,ngaygiaohang)"
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			PreparedStatement st = con.prepareStatement(sql);
			st.setString(1, t.getMaDonHang());
	...
	public int update(DonHang t) {
		...
		String sql = "UPDATE donhang" + " SET " + "khachhang=?" + ", diachimuahang=?" + ",diachinhanhang=?"
	...
	DonHang tg = tgd.selectById(new DonHang("M1", null, null, null, null, null, null, 0, 0, null, null));
